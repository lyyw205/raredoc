...
async function main() {
  ...
  const jpL = new Set((await prisma.regionCard.findMany({ where: { setId: { in: jpSets } }, select: { cardId: true } })).map((x) => x.cardId));
  const en = await prisma.regionCard.findMany({ where: { setId: enSet }, select: { number: true, name: true, cardId: true, card: { select: { supertype: true, pokedexNumbers: true } } } });
  let mism = 0, merged = 0;
  for (const e of en) { if (!jpL.has(e.cardId)) continue; merged++; if (!isPokemonSupertype(e.card.supertype)) continue; const ed = enDex.get(e.number); const ld = e.card.pokedexNumbers?.[0]; if (ed != null && ld != null && ed !== ld) { mism++; console.log(`⚠ EN#${e.number} ${e.name} enDex=${ed}≠LC ${ld}`); } }
