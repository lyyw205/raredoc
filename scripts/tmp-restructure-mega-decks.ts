 ...
 *   1) SetGroup mega-start-deck-100 신설 (era=MEGA-SP)
...
const NEW_JP_SETS = [
  { id: "jp-tcg-MC", code: "MC", name: "スタートデッキ100 バトルコレクション", releaseDate: new Date("2025-12-19"), setGroupId: "mega-start-deck-100", cardCount: 0 },
  { id: "jp-tcg-MBD", code: "MBD", name: "スターターセットMEGA メガディアンシーex", releaseDate: new Date("2025-09-05"), setGroupId: "mega-decks", cardCount: 0 },
  { id: "jp-tcg-MBG", code: "MBG", name: "スターターセットMEGA メガゲンガーex", releaseDate: new Date("2025-09-05"), setGroupId: "mega-decks", cardCount: 0 },
  { id: "jp-tcg-MA", code: "MA", name: "プレミアムトレーナーボックスMEGA", releaseDate: new Date("2025-08-01"), setGroupId: "mega-goods", cardCount: 0 },
];
...
async function main() {
  ...
  // 1) 새 그룹
  const exists = await prisma.setGroup.findUnique({ where: { id: NEW_GROUP.id } });
  console.log(`[1] SetGroup ${NEW_GROUP.id}: ${exists ? "이미 존재(스킵)" : "생성"} — ${NEW_GROUP.nameKo}`);
  if (APPLY && !exists) await prisma.setGroup.create({ data: NEW_GROUP });
  ...
  console.log(`[2] kr-mc Set → ${NEW_GROUP.id} 이동, 소속 LC ${mcLcs.length}개 setGroupId 이동`);
  if (APPLY) {
    await prisma.set.update({ where: { id: "kr-mc" }, data: { setGroupId: NEW_GROUP.id } });
    await prisma.logicalCard.updateMany({
      where: { id: { in: mcLcs.map((l) => l.id) } },
      data: { setGroupId: NEW_GROUP.id },
    });
  }
  ...
  for (const s of NEW_JP_SETS) {
    ...
    console.log(`[3] Set ${s.id} (code=${s.code} → group=${s.setGroupId}): ${ex ? "이미 존재(스킵)" : "생성"} — ${s.name}`);
    if (APPLY && !ex) {
