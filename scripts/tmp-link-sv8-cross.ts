...
async function main() {
  ...
  for (const [jpSet, jpNum, enSet, enNum] of PAIRS) {
    ...
    const en = await prisma.cardLocale.findFirst({ where: { setId: enSet, number: enNum }, select: { id: true, name: true, logicalCardId: true, logicalCard: { select: { illustrator: true, locales: { select: { region: true } } } } } });
