 ...
 *
 * - era 추론은 **이름 우선**(잘못 묶인 cardPack.era 교정). 예: "스칼렛&바이올렛 랜덤 스타트 덱"(og-s1w 소속) → SV.
...
async function main() {
  const allSets = await prisma.set.findMany({
    select: { id: true, name: true, region: true, cardPackId: true, cardPack: { select: { id: true, era: true } } },
  });
  // group → its sets (for dedicated detection)
  const byGroup = new Map<string, typeof allSets>();
  for (const s of allSets) { const k = s.cardPackId ?? "(null)"; if (!byGroup.has(k)) byGroup.set(k, []); byGroup.get(k)!.push(s); }
  ...
  for (const s of allSets) {
    ...
    if (s.cardPack?.era === "SV-SP" || s.cardPackId === "sv-decks" || s.cardPackId === "sv-goods") continue;
    const era = eraOf(s.name, s.cardPack?.era);
  ...
  if (!DRY) for (const g of usedGroups) {
    const m = groupMeta[g];
    await prisma.cardPack.upsert({ where: { id: g }, update: { era: m.era, nameKo: m.nameKo, order: m.order }, create: { id: g, era: m.era, nameKo: m.nameKo, order: m.order, releaseDate: null } });
  }
  ...
  for (const g of srcGroups) {
    const remain = DRY ? (byGroup.get(g) ?? []).filter((s) => !movedSetIds.has(s.id)).length : await prisma.set.count({ where: { cardPackId: g } });
    if (remain === 0) { emptied.push(g); if (!DRY) await prisma.cardPack.delete({ where: { id: g } }).catch(() => {}); }
  }
