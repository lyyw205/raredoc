/**
 * LogicalCard.cardPackId 백필 — 도감은 LogicalCard.cardPackId 로 그룹의 카드를 모은다.
 * EN 정체성 병합(merge-en-identity) 시 EN 로케일을 JP anchor LC(cardPackId 누락)로 옮기면서
 * 해당 그룹 카드가 도감에서 사라짐 → 그 그룹들의 LC 에 로케일 세트의 cardPackId 를 채운다.
...
async function main() {
  ...
  for (const s of await prisma.set.findMany({ select: { id: true, cardPackId: true } })) setGroupOf.set(s.id, s.cardPackId);

  const lcs = await prisma.logicalCard.findMany({ select: { id: true, cardPackId: true, locales: { select: { setId: true } } } });
  ...
  for (const lc of lcs) {
    ...
    if (lc.cardPackId === target) continue;
    fix++; perGroup.set(target, (perGroup.get(target) ?? 0) + 1);
    if (APPLY) await prisma.logicalCard.update({ where: { id: lc.id }, data: { cardPackId: target } });
  }
  console.log(`${APPLY ? "★적용" : "(dry)"} | cardPackId 교정 ${fix} (복수그룹스킵 ${ambiguous})`);
