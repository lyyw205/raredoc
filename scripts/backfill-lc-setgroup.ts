/**
 * Card.cardPackId 백필 — 도감은 Card.cardPackId 로 그룹의 카드를 모은다.
...
async function main() {
  ...
  const lcs = await prisma.card.findMany({ select: { id: true, cardPackId: true, locales: { select: { setId: true } } } });
  ...
  for (const lc of lcs) {
    ...
    if (APPLY) await prisma.card.update({ where: { id: lc.id }, data: { cardPackId: target } });
  }
