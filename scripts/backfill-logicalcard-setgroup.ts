/**
 * orphan Card (cardPackId=null) 의 cardPackId 를 primarySet 의 cardPackId 로 백필.
...
async function main() {
  const before = await prisma.card.count({ where: { cardPackId: null } });
  ...
  const after = await prisma.card.count({ where: { cardPackId: null } });
  const distinctSg = await prisma.card.findMany({
    where: { cardPackId: { not: null } },
    distinct: ["cardPackId"],
    select: { cardPackId: true },
  });
  console.log(`잔여(cardPackId=null): ${after}`);
  console.log(`Card 연결된 CardPack 수: ${distinctSg.length}`);
}
