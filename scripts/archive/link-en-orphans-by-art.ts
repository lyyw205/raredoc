...
//   - 대상은 cardId 가 lc-orphan* 인 EN 로케일뿐 → 형제 소유 카드는 구조적으로 못 건드림.
...
async function main() {
  ...
  const anchors = await prisma.card.findMany({
  ...
  // orphan EN 버킷 — "진짜 미연결"만: cardId 접두사(lc-orphan)로는 부족하다.
  ...
  const orphans = await prisma.regionCard.findMany({
    where: { region: "EN", setId: { in: enSets },
      card: { supertype: "Pokémon", locales: { none: { region: "JP" } } } },
    select: { id: true, setId: true, number: true, numberInt: true, name: true,
      card: { select: { illustrator: true, pokedexNumbers: true, subtypes: true } } },
  });
  const oBuckets = new Map<string, typeof orphans>();
  for (const o of orphans) { const k = bk(o.card.pokedexNumbers, o.card.illustrator); (oBuckets.get(k) ?? oBuckets.set(k, []).get(k)!).push(o); }
  ...
  for (const [k, os] of oBuckets) {
    ...
    for (const o of os) {
      ...
      let ai = as.findIndex((a, i) => !used.has(i) && subKey(a.subtypes) === subKey(o.card.subtypes));
  ...
  if (APPLY) {
    let n = 0;
    for (const l of links) { await prisma.regionCard.update({ where: { id: l.id }, data: { cardId: l.to } }); if (++n % 25 === 0) console.log(`  …${n}/${links.length}`); }
