...
async function main() {
  console.log(`【P4a art메타→RegionCard】${APPLY ? " ★APPLY" : " (dry-run)"}`);
  ...
  console.log(`  RegionCard ${totalCL} · 부모 rarity보유 ${parentRarity} · 부모 subtypes보유 ${parentSub}`);
  ...
  console.log(`  ✅ 복제 ${n} RegionCard`);

  // 검증: RegionCard ≠ 부모 (rarity/subtypes 불일치 = 0 이어야 정상)
