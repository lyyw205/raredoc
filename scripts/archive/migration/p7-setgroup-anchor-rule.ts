// ── P7-S1 진단2: cardPackId 가 "앵커 locale(JP>KR>EN) 의 Set.setGroupId" 규칙으로 도출되는가 ──
// ③복수·④불일치 케이스가 이 규칙으로 설명되면 cardPackId 는 P2 없이 도출가능(앵커규칙 재배선).
...
async function main() {
  ...
  const anchorGroup = (lc: typeof lcs[0]) => {
    for (const r of prio) {
      const hit = lc.locales.find((l) => l.region === r && l.set?.cardPackId);
      if (hit) return hit.set!.cardPackId!;
    }
    // 앵커 없으면 아무 locale의 그룹
    return lc.locales.find((l) => l.set?.cardPackId)?.set?.cardPackId ?? null;
  };
  ...
  for (const lc of lcs) {
    ...
    if (d === lc.cardPackId) match++;
    else { mismatch++; if (mm.length < 20) {
      const locs = lc.locales.map((l) => `${l.region}:${l.set?.id}(${l.set?.cardPackId ?? "∅"})`).join(" ");
      mm.push(`${lc.id} cardPackId=${lc.cardPackId} ≠ 앵커도출=${d} | ${locs}`);
    } }
  }
  console.log(`대상 LC(cardPackId보유+locale보유) ${lcs.length}`);
