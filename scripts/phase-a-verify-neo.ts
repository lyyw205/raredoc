/**
 * Phase A Verification: NEO1~4 (323 cards) comprehensive audit
 *
 * Sections:
...
// tcgdex set IDs for NEO series (Japanese)
const NEO_TCGDEX_SET_MAP: Record<string, string> = {
  neo1: "neo1",
  neo2: "neo2",
  neo3: "neo3",
  neo4: "neo4",
...
async function sectionE(locales: RegionCardRow[], lcards: CardRow[]): Promise<MissingCardProbe[]> {
  ...
  for (const card of missing) {
    const setKey = card.setId.replace("jp-tcg-", ""); // e.g. "neo1"
    const tcgdexSet = NEO_TCGDEX_SET_MAP[setKey] ?? setKey;
    const numMatch = card.id.match(/-(\d+)$/);
    const numStr = numMatch ? numMatch[1].padStart(3, "0") : "000";
    const url = `https://api.tcgdex.net/v2/ja/cards/${tcgdexSet}-${numStr}`;
...
function buildReport(data: {
  ...
  lines.push(`# Phase A Verification: NEO1~4`);
  lines.push(`\n생성 일시: ${now}\n`);
  lines.push(`대상: NEO1~4 (일본판 Neo 시리즈, 1999~2001). 총 ${totalCards}장.\n`);
  ...
  lines.push(`\n각 Card 필드의 채움률 (%). `);
  lines.push(`※ nameKo는 NEO 시리즈 한국 미발매이므로 0% 예상.\n`);
  ...
  if (data.sectionE.length === 0) {
    ...
    const eHeaders = ["세트", "번호", "이름", "tcgdex 상태", "tcgdex 이름", "레어도", "타입", "DB 존재"];
  ...
  lines.push(`> **사유:** JP NEO 시리즈(neo1~4)는 EN에서 Neo Genesis, Neo Discovery, Neo Revelation, Neo Destiny로 분할·재편되었으며, 카드 번호 체계가 재배열되어 단순 ID 매핑 불가. 별도 대응 테이블 작성 후 진행 예정.\n`);

  // -------------------------
  // Section H
  // -------------------------
  lines.push(`## H) 버전 가용성 (RegionCard 언어 분포)`);
  lines.push(`\n각 Card의 RegionCard 언어 조합. NEO는 한국 미발매이므로 ja only 예상.\n`);
  ...
  if (eRecoverable.length > 0) {
    actions.push({ priority: "P1", action: `누락 카드 ${eRecoverable.length}건 tcgdex에 존재 → 이미지 소스 별도 확보 후 재시도` });
  }
  ...
  lines.push(`| 우선순위 | 액션 |`);
  lines.push(`| --- | --- |`);
  actions.push({ priority: "P3", action: `nameKo 미입력 (NEO 한국 미발매) — 필요 시 비공식 한국어명 별도 관리` });
  ...
  lines.push(``);
  lines.push(`---`);
  lines.push(`*자동 생성: scripts/phase-a-verify-neo.ts*`);
...
async function main() {
  console.log("=== Phase A Verification: NEO1~4 ===\n");

  const NEO_SET_IDS = ["jp-tcg-neo1", "jp-tcg-neo2", "jp-tcg-neo3", "jp-tcg-neo4"];

  const sets = await prisma.set.findMany({
    where: { id: { in: NEO_SET_IDS } },
  ...
  const lcards = await prisma.card.findMany({
    where: { primarySetId: { in: NEO_SET_IDS } },
    select: {
      id: true,
      primarySetId: true,
      primaryNumber: true,
  ...
  const outPath = path.join(__dirname, "..", "docs", "phase-a-verification-neo.md");
  ...
  console.log(`G) EN cross-check: DEFERRED (no clean EN equivalent for NEO)`);
