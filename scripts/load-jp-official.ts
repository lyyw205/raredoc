...
async function main() {
  ...
  for (const c of cards) {
    ...
    if (supertype === "Pokémon") { if (c.dexId) dexTcg++; else if (dexFromJa(c.jaName, ja)) dexJa++; else { dexNone++; noDex.push(`#${c.number} ${c.jaName}`); } }
  }
  ...
  for (const c of cards) {
    ...
    let dex: number[] = [];
    if (c.dexId) dex = [c.dexId]; else if (supertype === "Pokémon") { const x = dexFromJa(c.jaName, ja); if (x) dex = [x]; }
