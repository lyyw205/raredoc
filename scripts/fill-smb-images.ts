/**
 * プレミアムトレーナーボックス (jp-smb, sm-decks) #7-18 이미지 누락 채우기.
 *
 * 배경: jp-smb #1-6 이미지 보유, #7-18(12장, 트레이너/스타디움+에너지) imageLarge/Small=NULL.
 *   박스 번들 XY 트레이너 재록판이라 공식 이미지가 SM-XY/ 디렉토리. (SMF/SMG #13+ 와 동일 패턴)
 * 출처: pokemon-card.com 공식 (pg=507-508=SMB). 이름 매핑 12/12(문제 없음). 12장 몽타주 시각검증 완료.
 *   맵 tmp/smb-map.json (number→공식 URL, 경로 정규화).
 *
 * 동작: 공식 다운 → webp large(q90)+245 small(q80) → R2 sm-decks/ja/{size}/jp-smb/{n}.webp
 *   → 기존 RegionCard imageLarge/Small UPDATE. ★이미지 전용, 정체성·연결 불변.
 *
 * dry: npx tsx scripts/fill-smb-images.ts
 * 적용: npx tsx scripts/fill-smb-images.ts --apply --allow-protected   (sm-decks 동결)
 */
...
const SET = "jp-smb", PACK = "sm-decks";
const MAP_PATH = "tmp/smb-map.json";
...
async function main() {
  assertWritable([PACK], { allow: hasAllowProtectedFlag(), dryRun: !APPLY, tool: "fill-smb-images" });
  const map: Item[] = JSON.parse(fs.readFileSync(MAP_PATH, "utf8"));
  console.log(`${APPLY ? "APPLY" : "DRY"} fill-smb-images | ${map.length}장 (#7-18 이미지 필)`);
  ...
  if (fails.length) fs.writeFileSync("tmp/smb-fill-failed.json", JSON.stringify(fails, null, 2));
  console.log(`\n=== 결과 === ok=${stats.ok} fail=${stats.fail}`);
  const cov = await prisma.regionCard.count({ where: { setId: SET, numberInt: { gte: 7, lte: 18 }, imageLarge: { not: null } } });
  console.log(`DB #7-18 이미지 보유: ${cov}/12`);
}
