...
async function main() {
  ...
  // 빈 LC 를 가리키는 locale 기반 참조(ExternalIdMapping/CollectionItem/Trade)는 regionCardId(여전히 유효)의 현재 LC 로 재지정.
  //   Ruling(LC전용) 또는 regionCardId 로 재홈 불가한 참조가 있으면 안전하게 중단.
  ...
  const cit = await prisma.collectionItem.findMany({ where: { cardId: { in: emptyIds } }, select: { id: true, regionCardId: true } });
  const trd = await prisma.trade.findMany({ where: { cardId: { in: emptyIds } }, select: { id: true, regionCardId: true } });
  const allLoc = [...new Set([...ext.map((m) => m.regionCardId), ...cit.map((m) => m.regionCardId), ...trd.map((m) => m.regionCardId)].filter(Boolean) as string[])];
  ...
  const citBad = cit.filter((m) => !resolve(m.regionCardId)), trdBad = trd.filter((m) => !resolve(m.regionCardId));
  ...
  if (ruling > 0 || citBad.length + trdBad.length > 0) { console.log("⚠️ 재홈 불가 참조(LC전용/regionCardId없음) — 중단(수동확인)"); await prisma.$disconnect(); return; }
  ...
  for (const m of cit) { const to = resolve(m.regionCardId); if (to) await prisma.collectionItem.update({ where: { id: m.id }, data: { cardId: to } }); }
  for (const m of trd) { const to = resolve(m.regionCardId); if (to) await prisma.trade.update({ where: { id: m.id }, data: { cardId: to } }); }
