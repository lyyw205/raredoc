/**
 * 強化パック ロストリンク (jp-tcg-LL, og-ll) #035·#036 이미지 누락 채우기 — LEGEND 상·하 1쌍.
 *   #035 ダークライ&クレセリアLEGEND (상반부, Darkrai, HP150)
 *   #036 ダークライ&クレセリアLEGEND (하반부, Cresselia, 기술)
 *
 * 배경: 정체성 이미 존재. ★LEGEND 쌍은 이름이 같아 이름검증으로 상/하 못 가림 → 확대 시각검증:
 *   #035=상(HP150+이름배너+Darkrai 상단) / #036=하(기술텍스트+Cresselia) 확인 완료(번호순=상→하, 사용자 순서 일치).
 * 출처: 사용자 제공 tcgcollector 이미지(URL).
 *
 * 동작: 다운 → webp large(q90)+245 small(q80) → R2 og-ll/ja/{size}/jp-tcg-LL/{NNN}.webp
 *   → 기존 RegionCard imageLarge/Small UPDATE. ★이미지 전용, 정체성·연결 불변.
 *
 * dry: npx tsx scripts/fill-ll-legend.ts
 * 적용: npx tsx scripts/fill-ll-legend.ts --apply
 */
...
const SET = "jp-tcg-LL", PACK = "og-ll";

const CARDS = [
  { number: "035", name: "ダークライ&クレセリアLEGEND", img: "https://static.tcgcollector.com/content/images/12/54/4b/12544bf6039254ade634d0da104bbb5b06fa56ba4c41ff56920c2ea787b3ffd2.webp" },
  { number: "036", name: "ダークライ&クレセリアLEGEND", img: "https://static.tcgcollector.com/content/images/99/f3/6d/99f36dcae8b7925889e6628ed57c4b4b3d4f0d36fb7d53fe0d3b224bc9a1d3ec.webp" },
];
...
async function main() {
  assertWritable([PACK], { allow: hasAllowProtectedFlag(), dryRun: !APPLY, tool: "fill-ll-legend" });
  console.log(`${APPLY ? "APPLY" : "DRY"} fill-ll-legend | ${CARDS.length}장 (LEGEND 상·하)`);
