/**
 * ガイアボルケーノ (jp-tcg-XY5, og-xy5) #001-#078 이미지 누락 채우기 (78장).
 *
 * 배경: jp-tcg-XY5 80장 중 78장 imageLarge/Small=NULL(#079·#080만 보유). 정체성(이름·supertype 등)은
 *   이미 DB에 존재 → 순수 이미지 채움.
 * 출처: pokemon-card.com 공식 (pg=415 = XY5-Bg 디렉토리). 매니페스트 data/collect/jp-xy5-images.json
 *   (번호→공식 이미지 URL, cardID 선형매핑). ★검증완: 파일명 ROMAJI가 DB명과 전수 일치 + 경계/중복/EX 12장
 *   몽타주 시각검증(메가보스고돌라·원시그란돈·듀얼ジュカイン·실드에너지 등 정확). 트윈 タイダルストーム 오염 0.
 *   ※ 공식 검색API JSON 의 cardThumbFile 이라 'large' 썸네일이 공식 제공 권위 경로.
 *
 * 동작: 공식 다운 → webp large(q90)+245 small(q80) → R2 og-xy5/ja/{size}/jp-tcg-XY5/{NNN}.webp
 *   → 기존 RegionCard imageLarge/Small UPDATE. ★이미지 전용, 정체성·연결 불변.
 *   ※ 공식 봇차단 회피: CONC=3 워커풀 + 요청간 딜레이.
 *
 * dry: npx tsx scripts/fill-xy5-images.ts
 * 적용: npx tsx scripts/fill-xy5-images.ts --apply
 */
...
const SET = "jp-tcg-XY5", PACK = "og-xy5";
const MAN = "data/collect/jp-xy5-images.json";
...
async function main() {
  assertWritable([PACK], { allow: hasAllowProtectedFlag(), dryRun: !APPLY, tool: "fill-xy5-images" });
  const man: Item[] = JSON.parse(readFileSync(MAN, "utf8"));
  console.log(`${APPLY ? "APPLY" : "DRY"} fill-xy5-images | ${man.length}장 (공식 pg415 XY5-Bg)`);
  // 사전 이름 일치 검증(전수)
  let nameMism = 0;
  for (const it of man) {
  ...
  if (fails.length) writeFileSync("tmp/xy5-fill-failed.json", JSON.stringify(fails, null, 2));
  console.log(`\n=== 결과 === ok=${stats.ok} fail=${stats.fail}${fails.length ? " (로그 tmp/xy5-fill-failed.json)" : ""}`);
  ...
  console.log(`jp-tcg-XY5 이미지 보유: ${cov}/${total}`);
}
