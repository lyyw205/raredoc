// ── P7-S1 회귀 게이트: 앵커 SQL(lcIdsInPack) ≡ 현 setGroupId 컬럼 (전 그룹 diff=0) ───
// 컬럼 드롭 전 안전 증명. 앵커규칙(JP>KR>EN) DISTINCT ON 으로 도출한 그룹별 LC집합이
// 현 LogicalCard.setGroupId 와 모든 그룹에서 동일한지 검증.
// 실행: npx tsx scripts/migration/p7-setgroup-rewire-regression.ts
import "dotenv/config";
import { prisma } from "../../src/lib/prisma";

// 앵커 도출 그룹: 각 LC의 JP>KR>EN locale 의 Set.setGroupId
...
async function main() {
  console.log("════ P7-S1 회귀 게이트: 앵커 SQL ≡ setGroupId ════\n");
  ...
  // 구: 현 컬럼 (lc → setGroupId)
  const cur = await prisma.logicalCard.findMany({ where: { setGroupId: { not: null } }, select: { id: true, setGroupId: true } });

  // 그룹별 집합 비교
  const oldByGrp = new Map<string, Set<string>>();
  for (const c of cur) { const g = c.setGroupId!; (oldByGrp.get(g) ?? oldByGrp.set(g, new Set()).get(g)!).add(c.id); }
  ...
  console.log(`\n${diffGroups === 0 ? "✅ diff=0 — 앵커 SQL 이 setGroupId 와 완전 동치. 재배선·드롭 안전." : "🔴 diff 발견 — 재배선 보류"}`);
