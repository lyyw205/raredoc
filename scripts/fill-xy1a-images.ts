/**
 * コレクションX (jp-tcg-XY1a, og-xy1a) #001-#063 이미지 누락 채우기 (63장, 전부).
 *   コレクションY(XY1b)의 쌍둥이 팩(제르네아스/X측). 동일 절차.
 *
 * 배경: jp-tcg-XY1a 63장 전부 imageLarge/Small=NULL. 정체성 이미 존재 → 순수 이미지 채움.
 * 출처: pokemon-card.com 공식 (pg=404 = XY1-Bx 디렉토리). 매니페스트 data/collect/jp-xy1a-images.json.
 *   pg-scan → cardID 정렬 매핑(본탄 #001-060=cardID 30046-30105, 재판 #061-063=35758-35760).
 *   ★검증완: 파일명 ROMAJI가 DB명과 전수 일치(휴리스틱 경고 0) + EX/경계/재판 8장 몽타주 시각검증
 *   (메가후시기바나·에어무드EX·제르네아스EX 노멀+풀아트재판 정확). 트윈 コレクションY(XY1-By) 오염 0.
 *
 * 동작: 공식 다운 → webp large(q90)+245 small(q80) → R2 og-xy1a/ja/{size}/jp-tcg-XY1a/{NNN}.webp
 *   → 기존 RegionCard imageLarge/Small UPDATE. ★이미지 전용, 정체성·연결 불변. (봇차단 회피 CONC=3+딜레이)
 *
 * dry: npx tsx scripts/fill-xy1a-images.ts
 * 적용: npx tsx scripts/fill-xy1a-images.ts --apply
 */
...
const SET = "jp-tcg-XY1a", PACK = "og-xy1a";
const MAN = "data/collect/jp-xy1a-images.json";
...
async function main() {
  assertWritable([PACK], { allow: hasAllowProtectedFlag(), dryRun: !APPLY, tool: "fill-xy1a-images" });
  const man: Item[] = JSON.parse(readFileSync(MAN, "utf8"));
  console.log(`${APPLY ? "APPLY" : "DRY"} fill-xy1a-images | ${man.length}장 (공식 pg404 XY1-Bx)`);
  let nameMism = 0;
  for (const it of man) {
  ...
  if (fails.length) writeFileSync("tmp/xy1a-fill-failed.json", JSON.stringify(fails, null, 2));
  console.log(`\n=== 결과 === ok=${stats.ok} fail=${stats.fail}${fails.length ? " (로그 tmp/xy1a-fill-failed.json)" : ""}`);
  ...
  console.log(`jp-tcg-XY1a 이미지 보유: ${cov}/${total}`);
}
