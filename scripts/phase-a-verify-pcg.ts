/**
 * Phase A Verification: PCG1~9 (720 cards) comprehensive audit
 *
 * Sections:
 ...
 *
 * Run: npx tsx scripts/phase-a-verify-pcg.ts
 * Output: docs/phase-a-verification-pcg.md
 */
...
async function sectionC(cards: RegionCardRow[], sets: { id: string; cardCount: number | null }[]): Promise<ContiguityResult[]> {
  ...
  for (const card of cards) {
    ...
    // PCG RegionCard IDs: jp-tcg-PCG1-001 (padded)
...
async function sectionE(locales: RegionCardRow[], lcards: CardRow[]): Promise<MissingCardProbe[]> {
  ...
  for (const card of missing) {
    const setKey = card.setId.replace("jp-tcg-", "").toLowerCase(); // e.g. "pcg1"
    // PCG numbers are padded: "001"
    const numMatch = card.id.match(/-(\d+)$/);
    const numStr = numMatch ? numMatch[1].padStart(3, "0") : "000";
    const url = `https://api.tcgdex.net/v2/ja/cards/${setKey.toUpperCase()}-${numStr}`;
    const { status, body } = await curlJson<{ name?: string; rarity?: string }>(url);
...
async function sectionH(lcards: CardRow[]): Promise<LocaleStats[]> {
  ...
  const localeRows = await prisma.regionCard.findMany({
    ...
    select: { cardId: true, language: true, setId: true },
  });
  ...
  for (const card of lcards) {
    ...
    if (langsArr.length > 1) s.multiLocale.push({ id: card.id, langs: langsArr });
  }
...
function buildReport(data: {
  ...
  const SETS = ["PCG1","PCG2","PCG3","PCG4","PCG5","PCG6","PCG7","PCG8","PCG9"];
  const FIELDS: FieldKey[] = ["hp", "types", "attacks", "abilities", "subtypes", "illustrator", "rarityId", "pokedexNumbers", "supertype", "nameKo"];
  const lines: string[] = [];

  const now = new Date().toISOString().replace("T", " ").substring(0, 19) + " UTC";
  const totalCards = data.setDefs.reduce((a, s) => a + (s.cardCount ?? 0), 0);
  lines.push(`# Phase A Verification: PCG1~9`);
  lines.push(`\n생성 일시: ${now}\n`);
  lines.push(`대상: PCG1~9 (일본판 PCG 시리즈, 2004~2006). 총 ${totalCards}장.\n`);
  ...
  if (data.sectionE.length === 0) {
    ...
    const eHeaders = ["세트", "번호", "이름", "tcgdex 상태", "tcgdex 이름", "레어도", "DB 존재"];
    const eRows = data.sectionE.map(p => [
      p.set, p.number, p.name, String(p.tcgdexStatus),
      p.tcgdexName ?? "—", p.tcgdexRarity ?? "—", p.inDb ? "O" : "X",
    ]);
    ...
    lines.push(`\n**요약:** tcgdex 200=${e200}, 404=${e404}`);
    if (e200 > 0) lines.push(`> tcgdex에 존재하는 카드(${e200}건)는 이미지 소스 확보 후 재시도 가능.`);
  }
  ...
  lines.push(`\n각 Card의 RegionCard 언어 조합. PCG는 한국 미발매이므로 ja only 예상.\n`);
  ...
  if (noIllustrator.length > 0)
    actions.push({ priority: "P3", action: `illustrator 채움률 낮음: ${noIllustrator.join(", ")} — Bulbapedia 재시도` });
  ...
  lines.push(``);
  lines.push(`---`);
  lines.push(`*자동 생성: scripts/phase-a-verify-pcg.ts*`);
...
async function main() {
  console.log("=== Phase A Verification: PCG1~9 ===\n");

  const PCG_SET_IDS = [
    "jp-tcg-PCG1","jp-tcg-PCG2","jp-tcg-PCG3","jp-tcg-PCG4","jp-tcg-PCG5",
    "jp-tcg-PCG6","jp-tcg-PCG7","jp-tcg-PCG8","jp-tcg-PCG9",
  ];

  const sets = await prisma.set.findMany({
    where: { id: { in: PCG_SET_IDS } },
    select: { id: true, name: true, cardCount: true },
    orderBy: { id: "asc" },
  });

  const locales = await prisma.regionCard.findMany({
    where: { setId: { in: PCG_SET_IDS } },
    select: { id: true, name: true, setId: true, imageSmall: true },
    orderBy: { id: "asc" },
  });

  const lcards = await prisma.card.findMany({
    where: { primarySetId: { in: PCG_SET_IDS } },
    select: {
  ...
  console.log(`Loaded: ${sets.length} sets, ${locales.length} RegionCard, ${lcards.length} Card`);
  ...
  const outPath = path.join(__dirname, "..", "docs", "phase-a-verification-pcg.md");
