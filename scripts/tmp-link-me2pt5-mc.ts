...
async function main() {
  ...
  const mcAll = await prisma.regionCard.findMany({
    ...
    select: { id: true, number: true, name: true, cardId: true,
      card: { select: { supertype: true, pokedexNumbers: true, illustrator: true, locales: { where: { region: "EN" }, select: { id: true } } } } },
  });
  const mc = mcAll.filter(m => !m.card?.locales.length);

  // 2) me2pt5 orphan
  const en = await prisma.regionCard.findMany({
    where: { setId: "en-tcg-me2pt5", cardId: { startsWith: "lc-orphan-en-tcg-me2pt5-" } },
    select: { id: true, number: true, name: true, cardId: true,
      card: { select: { supertype: true, pokedexNumbers: true, illustrator: true } } },
  });
  ...
  for (const m of mc) {
    if (!POKE.includes(m.card?.supertype ?? "")) continue;
    const d = m.card?.pokedexNumbers?.[0]; if (d == null || !m.card?.illustrator) continue;
    const k = key(d, m.card.illustrator);
    (jpByKey.get(k) ?? jpByKey.set(k, []).get(k))!.push(m);
  }
  const enByKey = new Map<string, typeof en>();
  for (const e of en) {
    if (!POKE.includes(e.card?.supertype ?? "")) continue;
    const d = e.card?.pokedexNumbers?.[0]; if (d == null || !e.card?.illustrator) continue;
    const k = key(d, e.card.illustrator);
  ...
  for (const [k, el] of enByKey) {
    ...
    if (el.length === 1 && jl.length === 1) {
      pairs.push({ enId: el[0].id, enName: el[0].name, lcid: jl[0].cardId!, jpNum: jl[0].number, jpName: jl[0].name, via: "dex+일러" });
  ...
  const enTr = en.filter(e => !POKE.includes(e.card?.supertype ?? "") && !usedEn.has(e.id));
  const jpTr = mc.filter(m => !POKE.includes(m.card?.supertype ?? "") && !usedJp.has(m.id));
  ...
  for (const [name, el] of enByName) {
    ...
    for (let i = 0; i < n; i++) {
      pairs.push({ enId: es[i].id, enName: name, lcid: jl[i].cardId!, jpNum: jl[i].number, jpName: jl[i].name, via: es.length > 1 ? "사전+구간zip" : "사전" });
  ...
  if (APPLY) {
    for (const p of pairs) {
      const cur = await prisma.regionCard.findUnique({ where: { id: p.enId }, select: { cardId: true } });
      await prisma.regionCard.update({ where: { id: p.enId }, data: { cardId: p.lcid } });
      if (cur?.cardId?.startsWith("lc-orphan-en-tcg-me2pt5")) {
        const left = await prisma.regionCard.count({ where: { cardId: cur.cardId } });
        if (left === 0) await prisma.card.delete({ where: { id: cur.cardId } });
      }
    }
    console.log(`\n★적용 ${pairs.length}`);
  } else console.log("\n(dry — --apply 로 적용)");
