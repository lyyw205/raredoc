...
const items = [
  ["037","037 Zoroark BREAK (Darkness 140)"],
  ["043","043 Florges BREAK (Fairy 140)"],
];
(async () => {
  const W = 500, H = 699, GAP = 12, LABEL = 32;
  const cells = await Promise.all(items.map(async ([n, label]) => {
    const img = await sharp(`tmp/xy8a/${n}.jpg`).resize(W, H, { fit: "contain", background: "#fff" }).toBuffer();
  ...
  await sharp({ create: { width: W * 2 + GAP, height: cellH, channels: 3, background: "#ddd" } })
    .composite(cells.map((c, i) => ({ input: c, top: 0, left: i * (W + GAP) }))).png().toFile("tmp/xy8a/montage.png");
  console.log("ok");
})();
