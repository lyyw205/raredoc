...
async function main() {
  const tcg: any[] = JSON.parse(readFileSync("tmp/neo3/tcg.json", "utf8"));
  const byNum = new Map(tcg.map((x) => [x.number, x]));
  const db = await prisma.regionCard.findMany({
    where: { setId: "jp-tcg-neo3", region: "JP" },
  ...
  for (const r of db) {
    ...
    const sp = await prisma.cardSpecies.findMany({
      ...
      select: { species: { select: { nameEn: true, nameJa: true } } },
    });
    const spNames = sp.map((s) => s.species.nameEn?.toLowerCase()).filter(Boolean) as string[];
    if (!t) {
      mism.push({ n: r.number, db: r.name, reason: "NO_TCG" });
      continue;
    }
  ...
  console.log("species-match=" + match + " noDex(trainer/energy)=" + noDex + " mismatch=" + mism.length);
