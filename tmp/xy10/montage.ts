...
const items = [
  ["012","012 Delphox BREAK (Fire 180)"],
  ["017","017 Omastar BREAK (Water 140)"],
  ["049","049 Bronzong BREAK (Metal 130)"],
  ["059","059 Lugia BREAK (Colorless 150)"],
];
(async () => {
  const W = 460, H = 643, GAP = 10, LABEL = 30;
  const cells = await Promise.all(items.map(async ([n, label]) => {
    const img = await sharp(`tmp/xy10/${n}.jpg`).resize(W, H, { fit: "contain", background: "#fff" }).toBuffer();
  ...
  await sharp({ create: { width: W * 4 + GAP * 3, height: cellH, channels: 3, background: "#ddd" } })
    .composite(cells.map((c, i) => ({ input: c, top: 0, left: i * (W + GAP) }))).png().toFile("tmp/xy10/montage.png");
  console.log("ok");
})();
